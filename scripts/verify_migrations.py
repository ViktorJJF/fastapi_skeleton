...
def check_import(module_path, expected_attributes):
    ...
    try:
        ...
        if missing:
            print(
                f"{Fore.RED}✗ Module {module_path} is missing attributes: {', '.join(missing)}{Style.RESET_ALL}"
            )
            return False
        else:
            print(
                f"{Fore.GREEN}✓ Module {module_path} contains all required attributes{Style.RESET_ALL}"
            )
            return True
...
def check_alembic_cmd():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(
                f"{Fore.RED}✗ Alembic command failed: {result.stderr}{Style.RESET_ALL}"
            )
            return False
...
def main():
    ...
    success &= check_file_exists(
        "migrations/script.py.mako", "migrations/script.py.mako exists"
    )

    # Check for directories
    print("\nChecking directories:")
    success &= check_directory_exists(
        "migrations/versions", "migrations/versions directory exists"
    )
    ...
    success &= check_file_exists(
        "app/database/connection.py", "database connection file exists"
    )
    ...
    if os.path.isfile("migrations/env.py"):
        try:
            ...
            if model_imports:
                print(
                    f"{Fore.GREEN}✓ Environment correctly imports models{Style.RESET_ALL}"
                )
            else:
                print(
                    f"{Fore.YELLOW}⚠ No model imports found in env.py. Autogeneration may not detect all models.{Style.RESET_ALL}"
                )
                success = False

        except Exception as e:
            print(
                f"{Fore.RED}✗ Error checking model imports: {str(e)}{Style.RESET_ALL}"
            )
            success = False
