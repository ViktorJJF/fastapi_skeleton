...
def main():
    ...
    empty_parser = subparsers.add_parser("create-empty", help="Create an empty migration for manual operations")
    ...
    branch_parser = subparsers.add_parser("create-branch", help="Create a branch migration from a specific parent")
    ...
    migrate_parser.add_argument("revision", nargs="?", help="Target revision (default: head)")
    migrate_parser.set_defaults(func=migrate)
    
    # Rollback command
    rollback_parser = subparsers.add_parser("rollback", help="Rollback migrations")
    rollback_parser.add_argument("step", nargs="?", type=int, help="Number of migrations to rollback (default: 1)")
    ...
    history_parser.add_argument("-v", "--verbose", action="store_true", help="Show verbose history")
    history_parser.add_argument("-r", "--range", help="Range of revisions (e.g., 'base:head', '-3:current')")
    history_parser.set_defaults(func=history)
    
    # Stamp command
    stamp_parser = subparsers.add_parser("stamp", help="Stamp the database with the given revision")
    ...
    sql_parser = subparsers.add_parser("sql", help="Generate SQL script for migrations without executing")
    sql_parser.add_argument("revision", nargs="?", help="Target revision (default: head)")
    sql_parser.add_argument("--output", "-o", help="Output file (default: migration.sql)")
    sql_parser.set_defaults(func=generate_sql)
    
    # Heads command
    heads_parser = subparsers.add_parser("heads", help="Show current branch heads")
    heads_parser.add_argument("-v", "--verbose", action="store_true", help="Show verbose output")
    heads_parser.set_defaults(func=heads)
    
    # Branches command
    branches_parser = subparsers.add_parser("branches", help="Show current branch points")
    branches_parser.add_argument("-v", "--verbose", action="store_true", help="Show verbose output")
