...
class UserBase(BaseModel):
    ...
    verified: Optional[bool] = Field(
        False, description="Whether the user email is verified"
    )
...
class UserCreate(UserBase):
    password: str = Field(
        ..., min_length=6, description="User's password (min  characters)"
    )
...
class UserUpdate(BaseModel):
    ...
    password: Optional[str] = Field(
        None, min_length=6, description="New password (optional, min 6 characters)"
    )
...
class User(UserBase):
    ...
    login_attempts: Optional[int] = Field(
        0, description="Number of failed login attempts"
    )
    block_expires: Optional[datetime] = Field(
        None, description="Timestamp until the user is blocked"
    )
