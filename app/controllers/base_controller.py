...
class BaseController(Generic[ModelType, CreateSchemaType, UpdateSchemaType, GetSchemaType]):
    """
    ...
    def setup_routes(self):
        ...
        @self.router.post("/", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED)
        async def create(
        ...
        async def update(
            ...
            try:
                ...
                if await self.item_exists_excluding_itself(db, valid_id, item_in.dict(exclude_unset=True)):
                    raise build_error_object(
                        status.HTTP_409_CONFLICT,
                        "Item with these unique fields already exists"
                    )
