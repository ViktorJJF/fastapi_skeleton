...
async def get_current_user(
    ...
    try:
        ...
        token_data = UserInToken(id=token_user_id, role=payload.role, email=payload.email)
    ...
    if security_scopes.scopes: # If specific scopes (roles) are required by the endpoint
...
def get_current_active_user(
    current_user: UserInToken = Security(get_current_user, scopes=[]) # No specific scope needed just to be logged in
) -> UserInToken:
...
def require_role(required_role: str):
    ...
    def role_checker(current_user: UserInToken = Security(get_current_user, scopes=[required_role])) -> UserInToken:
...
def require_roles(required_roles: list[str]):
    ...
    def roles_checker(current_user: UserInToken = Security(get_current_user, scopes=required_roles)) -> UserInToken:
