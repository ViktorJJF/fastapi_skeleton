...
def item_not_found(
    error: Optional[Exception], item: Any, message: str = "Item not found"
) -> None:
    """
...
def item_already_exists(
    error: Optional[Exception], item: Any, message: str = "Item already exists"
) -> None:
    """
...
async def handle_error(arg1: Any, error: Optional[Exception] = None) -> JSONResponse:
    ...
    if error is None:
        ...
        if isinstance(arg1, Exception):
            ...
            logger.warning(
                f"handle_error called with one argument, but it wasn't an Exception: {arg1}"
            )
    else:
        ...
        if isinstance(arg1, Request):
            ...
            logger.warning(
                "handle_error called with two arguments, but the first was not a FastAPI Request object."
            )
    ...
    if isinstance(actual_error, HTTPException):
        ...
        if request:
            ...
            if hasattr(request, "headers"):
                ...
                headers = {
                    k: (
                        "******"
                        if k.lower()
                        in ["authorization", "api-key", "x-api-key", "secret"]
                        else v
                    )
            ...
            try:
                ...
                logger.warning(
                    f"Could not read request body asynchronously: {body_exc}"
                )
        ...
        http_info = f"Validation Error Details:\n{error_details_str}\n\n"
    ...
    try:
        telegram_notifier.send_error_notification(
            notification_error_message, notification_traceback
        )
    ...
    response_content = {
        ...
        "errors": {
            "msg": (
                str(actual_error.detail)
                if isinstance(actual_error, HTTPException)
                else (
                    safe_serialize(actual_error.errors())
                    if isinstance(actual_error, RequestValidationError)
                    else str(actual_error)
                )
            )
        },
    }
